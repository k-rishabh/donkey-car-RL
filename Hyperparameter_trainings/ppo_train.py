...
class PPOAgent:
    def __init__(self, state_dim, action_dim, lr=3e-4, gamma=0.99, lam=0.95, clip_epsilon=0.2, 
        ...
        # For logging losses and metrics
    ...
    try:
        while episode < max_episodes:
            ...
            for t in range(max_timesteps):
                ...
                if timestep % update_timestep == 0:
                    ...
                    if len(agent.policy_loss_history) > 0:
                        policy_losses.append(agent.policy_loss_history[-1])
                        value_losses.append(agent.value_loss_history[-1])
                        total_losses.append(agent.total_loss_history[-1])
                        entropies.append(agent.entropy_history[-1])

                    # Clear memory and histories
                    memory.clear()
                    agent.policy_loss_history = []
                    agent.value_loss_history = []
                    agent.total_loss_history = []
                    agent.entropy_history = []
            ...
            if episode % log_interval == 0:
                ...
                # Log losses and entropy if available
                if len(policy_losses) > 0:
                    writer.add_scalar('Policy Loss', policy_losses[-1], episode)
                    writer.add_scalar('Value Loss', value_losses[-1], episode)
                    writer.add_scalar('Total Loss', total_losses[-1], episode)
                    writer.add_scalar('Entropy', entropies[-1], episode)
                    writer.add_scalar('Average Advantage', avg_advantages[-1], episode)
        ...
        try:
            ...
            # Plot policy loss, value loss, and total loss
            plt.figure()
            ...
            plt.legend()
            plt.grid(True)
            plt.savefig('ppo_training_losses.png')
            plt.show()
            logger.info("Loss plots saved.")

            # Plot entropy over updates
            plt.figure()
            plt.plot(range(1, len(entropies) + 1), entropies)
            plt.xlabel('Updates')
            plt.ylabel('Entropy')
            plt.title('PPO Training Entropy')
            plt.grid(True)
            plt.savefig('ppo_training_entropy.png')
            plt.show()
            logger.info("Entropy plot saved.")

            # Plot average advantages
            plt.figure()
            plt.plot(range(1, len(avg_advantages) + 1), avg_advantages)
            plt.xlabel('Updates')
            plt.ylabel('Average Advantage')
            plt.title('PPO Training Average Advantage')
            plt.grid(True)
            plt.savefig('ppo_training_avg_advantage.png')
            plt.show()
            logger.info("Average advantage plot saved.")
